...
CREATE TRIGGER insert_dt_trigger 
AFTER INSERT ON dt
FOR EACH ROW
BEGIN
    INSERT INTO dt (title, pcontent, type, bhk, stype, bedroom, bathroom, balcony, kitchen, hall, floor, size, price, location, city, state)
    VALUES (NEW.title, NEW.pcontent, NEW.type, NEW.bhk, NEW.stype, NEW.bedroom, NEW.bathroom, NEW.balcony, NEW.kitchen, NEW.hall, NEW.floor, NEW.size, NEW.price, NEW.location, NEW.city, NEW.state);
END //
